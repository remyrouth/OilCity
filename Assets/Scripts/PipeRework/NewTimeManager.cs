...
/// <summary>
/// COPIED FROM OLD TIME MANAGER
/// </summary>
public class NewTimeManager : Singleton<TimeManager>
{
#if UNITY_EDITOR
    ...
    private readonly Collection<INewFlowable> m_nodes = new();
    ...
    public void RegisterReceiver(MonoBehaviour receiver)
    {
        ...
        if (receiver is INewFlowable flow_node)
    ...
    private void HandleRegister(INewFlowable node)
    {
        var tr = node.GetTreeRelationship();
        var tentative_children = tr.GetTentative(Relation.Child);
        var tentative_parents = tr.GetTentative(Relation.Parent);
